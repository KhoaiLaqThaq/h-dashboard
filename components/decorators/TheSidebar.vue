        ...
        <template v-if="item.to">
          <nuxt-link class="side-menu" :to="item.to" :title="$t(item.title)" @click="toggleSidebar()">
            <div class="side-menu__icon" v-if="item.iconData">
              <font-awesome-icon :icon="item.iconData"></font-awesome-icon>
            </div>
            <span class="side-menu__title ps-1">{{ $t(item.label) }}</span>
          </nuxt-link>
        </template>
        <template v-else>
          <div class="nav-link side-menu cursor-pointer" :title="$t(item.title)" @click="toggleSidebar(item.id, false)">
            <div class="side-menu__icon" v-if="item.iconData">
